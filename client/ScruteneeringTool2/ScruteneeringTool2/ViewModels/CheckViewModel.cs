        ...
        private async void TryOCR()
        {
            ...
            using (var photoStream = new MemoryStream(_imageBytes))
            {
                try
                {
                    ...
                    var scrutineerClient = new RestClient("http://scrutineer-scrutineer.1d35.starter-us-east-1.openshiftapps.com");
                    var request = new RestRequest("api/checker", Method.POST);
                    request.AddParameter("standartCode", _ocrrawtext);
                    IRestResponse response = scrutineerClient.Execute(request);
